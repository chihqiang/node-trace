...
function parseBrowserInfo(userAgent: string): BrowserInfo {
  ...
  if (/Chrome/.test(userAgent)) {
    name = "Chrome";
    major = userAgent.match(/Chrome\/(\d+)/)?.[1] || "0";
    engine = "Blink";
  } else if (/Firefox/.test(userAgent)) {
    name = "Firefox";
    major = userAgent.match(/Firefox\/(\d+)/)?.[1] || "0";
    engine = "Gecko";
  } else if (/Safari/.test(userAgent) && !/Chrome/.test(userAgent)) {
    name = "Safari";
    major = userAgent.match(/Version\/(\d+)/)?.[1] || "0";
    engine = "WebKit";
  } else if (/Edge/.test(userAgent)) {
    name = "Edge";
    major = userAgent.match(/Edge\/(\d+)/)?.[1] || "0";
    engine = "Blink";
  } else if (/MSIE|Trident/.test(userAgent)) {
    name = "Internet Explorer";
    major = userAgent.match(/MSIE\s(\d+)|rv:(\d+)/)?.[1] || "0";
    engine = "Trident";
  }
